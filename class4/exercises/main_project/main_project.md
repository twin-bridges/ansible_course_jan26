* Build a firewall.
    ...
    Should use roles.
        * Should have a separate role for Gaia Components.
        * Should have separate roles for Mgmt Objects and Mgmt Policy
        * You might want to use a shared role for handlers (which would be a required dependency for your other
          roles in meta/main.yml.
...
-----
    ...
    * First configure management access from the Ansible Server and SmartConsole to not lock us out (from the Ansible Server).
        ...
        Allow "Any" access from (to the firewall/mgmt server itself):
    ...
    * Configure HTTP/HTTPS access from Any source to "Corp Web Server" on the DMZ
